...
public class PlayerAddToBalance extends ElementUCP {
    ...
    public void run(final ElementInfo info, final ScriptInstance instance) {
        ...
        if(!VaultUtil.hasVault()) {
            EZMessage.text("%prefix% &cVault is needed to use the PlayerAddToBalance element.").console();
            return;
        }
        if(VaultUtil.getEconomy() == null) {
            EZMessage.text("%prefix% &cThe Economy has to be setup to use the PlayerAddToBalance element. Do you have an Economy plugin installed?").console();
